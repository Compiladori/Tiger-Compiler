...
void Muncher::munchStatement(irt::Statement* stm){
    ...
    if(auto move_stm = dynamic_cast<irt::Move*>(stm)){
        ...
        if(mem_dst_stm and mem_src_stm){
            /* MOVE((MEM e1), (MEM e2)) */
    ...
    if(auto exp_stm = dynamic_cast<irt::Exp*>(stm)){
        if(auto call_exp = dynamic_cast<irt::Call*>(exp_stm)){
...
temp::Temp Muncher::munchExpression(irt::Expression* exp){
    ...
    if(auto call_exp = dynamic_cast<irt::Call*>(exp)){
        /* CALL(fun, args) */
        throw error::internal_error("Unexpected Call while munching an expression: shouldn't be present in a canonized IRT", __FILE__);
    }
